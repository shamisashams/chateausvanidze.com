    ...
    <div class="row">
        ...
        <form action="{{route('DictionaryUpdate', [$locale,$language->id])}}"method="POST" class="bg-white py-3 px-4 grid w-full grid-cols-2 gap-3">
            ...
            <div class="col-span-1 w-full">
                ...
                <input required type="text" value="{{$language->module}}" name="module" class="form-control" placeholder="@lang('language.module')"> 
                @error('module')
                <p>{{$message}}</p>
                @enderror 
            </div>
            ...
            <div class="border-t m-0 py-3 col-span-3">
                <button class="btn btn-primary" type="submit">@lang('language.update')</button>
            </div>
        </form>
    </div>
</div>
