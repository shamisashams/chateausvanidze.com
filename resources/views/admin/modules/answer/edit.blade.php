    ...
    <div class="col-span-3">
        <form action="{{route('AnswerUpdate', [$locale,$answer->id])}}" method="POST" class="bg-white py-3 px-4 grid w-full grid-cols-3 gap-3">
            ...
            <div class="col-span-1 w-full ">
                ...
                <input type="text" name="position" value="{{$answer->position}}" class="w-full py-2 px-3 bg-gray-100 rounded-md font-normal text-xs" placeholder="@lang('answer.position')"> 
            </div>
            <div class="col-span-1 w-full">
                <small>@lang('answer.slug')</small>
                <input required type="text" name="slug" value="{{$answer->slug}}" class="w-full py-2 px-3 bg-gray-100 rounded-md font-normal text-xs" placeholder="@lang('answer.slug')"> 
            </div>
            <div class="col-span-2 w-full">
                <small>@lang('answer.title')</small>
                <input required type="text" name="title" value="{{($answer->language()->where('language_id', $localization)->first()->title) ?? $answer->language()->first()->title}}" class="w-full py-2 px-3 bg-gray-100 rounded-md font-normal text-xs" placeholder="@lang('answer.title')"> <br>
            </div>
            <div class="col-span-1 w-full">
